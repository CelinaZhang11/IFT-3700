...
def pivot_months_pandas(data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Group by name and month, then sum the precipitation
    monthly = data.groupby(['name', 'month'])['precipitation'].sum().reset_index()

    # Pivot table 
    monthly = monthly.pivot(index='name', columns='month', values='precipitation').fillna(0)
    monthly = monthly.round(1)

    # Group by station and month, then count the observations
    counts = data.groupby(['name', 'month'])['precipitation'].count().reset_index()

    # Pivot table
    counts = counts.pivot(index='name', columns='month', values='precipitation').fillna(0)
...
def compute_pairwise(df: pd.DataFrame, func: callable) -> pd.DataFrame:
    ...
    **chaque paire de lignes du dataframe**.
    ...
    # Compute pairwise distances using scipy.spatial.pdist and scipy.spatial.squareform
    new_df = pd.DataFrame(squareform(pdist(df, func)), index=df.index, columns=df.index)
...
def compute_pairwise_distances(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Drop duplicates to ensure unique station entries
    df = df.drop_duplicates(subset=["name"])

    # Pivot dataframe so that lat/lon are columns and names are index
    latlon = df[["latitude", "longitude"]].set_index(df["name"])
    latlon = latlon.sort_index()

    new_df = compute_pairwise(latlon, geodesic)
...
def correlation(u, v) -> float:
    ...
    # Calculates mean and standard deviation of valid entries
    u_avg = u[idx].mean()
    v_avg = v[idx].mean()
    u_std = u[idx].std()
    v_std = v[idx].std()
    ...
    corr = ((u[idx] - u_avg) * (v[idx] - v_avg)).mean() / (u_std * v_std)
...
def compute_pairwise_correlation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Pivot dataframe so that dates are columns and names are index
    df = df.pivot(index="name", columns="date", values="precipitation")
    new_df = compute_pairwise(df, correlation)
...
def compute_pairwise_correlation_pandas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Calculates correlation
    new_df = df.pivot_table(index="name", columns="date", values="precipitation", aggfunc="sum").T.corr()
