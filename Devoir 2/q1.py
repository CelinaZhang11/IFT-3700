...
def contains_label(labels: pd.Series, label: str) -> pd.Series:
    ...
    les valeurs qui incluent `label` comme une correspondance exacte.
...
def get_correlation(labels: pd.Series, label_1: str, label_2: str) -> float:
    ...
    label_1_count = contains_label(labels, label_1)
    label_1_and_2_count = contains_label(label_1_count, label_2)
    
    return len(label_1_and_2_count) / len(label_1_count)


if __name__ == "__main__":
    print(count_labels("/m/04rlf,/m/06_fw,/m/09x0r"))
    print(convert_id("/m/0dgw9r"))
    print(convert_ids("/m/04rlf,/m/06_fw,/m/09x0r"))

    series = pd.Series([
        "Country Music|Skateboard|Speech",
        "Voice|Speech",
        "Music|Piano"
    ])
